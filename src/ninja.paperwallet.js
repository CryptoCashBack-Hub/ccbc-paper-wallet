ninja.wallets.paperwallet = {
	...
	// generate cryptocashback address, private key, QR Code and update information in the HTML
	// idPostFix: 1, 2, 3, etc.
	generateNewWallet: function (idPostFix) {
		if (ninja.wallets.paperwallet.encrypt) {
			...
			ninja.wallets.paperwallet.showArtisticWallet(idPostFix, cryptocashbackAddress, privateKeyWif);
		}
	},
	...
	testAndApplyVanityKey: function () {
		...
		if (!ninja.privateKey.isPrivateKey(suppliedKey)) {
			...
			var computedPublicAddress = new CryptoCashBack.ECKey(suppliedKey).getCryptoCashBackAddress();
			if (ninja.wallets.paperwallet.encrypt) {
				document.getElementById("busyblock").className = "busy";
	...
	templateArtisticHtml: function (i) {
		...
		var walletHtml =
							"<div class='coinIcoin'> <img id='coinImg' src='" + coinImgUrl + "' alt='currency_logo' /></div><div class='artwallet' id='artwallet" + i + "'>" +
								...
								"<div class='ccbcaddress' id='ccbcaddress" + i + "'></div>" +
	...
	showArtisticWallet: function (idPostFix, cryptocashbackAddress, privateKey) {
		var keyValuePair = {};
		keyValuePair["qrcode_public" + idPostFix] = cryptocashbackAddress;
        ...
        document.getElementById("ccbcaddress" + idPostFix).innerHTML = cryptocashbackAddress;
		document.getElementById("ccbcprivwif" + idPostFix).innerHTML = privateKey;
	},
